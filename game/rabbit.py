...
class RabbitGameAI:
    ...
    def __init__(self, obstacles, w=30 * BLOCK_SIZE, h=20 * BLOCK_SIZE, game_speed = 100, game_level = 1):
        self.w = w
        self.h = h
    ...
    def _generate_maze(self):
        ...
        if len(self.obstacles) != 0:
            for k in range(len(self.obstacles)):
                    x = self.obstacles[k][0]
                    y = self.obstacles[k][1]
                    maze[y][x] = 1
    ...
    def _move(self, action, who='RL'):
        if who == 'RL':
            ...
            if np.array_equal(action, [1, 0, 0]):   # 100
                new_dir = clockwise[idx]
            elif np.array_equal(action, [0, 1, 0]): #010
